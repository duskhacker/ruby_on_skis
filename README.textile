...
The aim of the wxRubyTemplate is to provide a starting structure within which to program your wxRuby project. The template provides a directory structure, a set of Rakefiles to manage, and most importantly, to _package_ your Ruby application for distribution as a standalone program when completed. For now, the template only supports packaging for Mac OSX and Windows, although various flavors of Linux should be easily accommodated and are planned for the future.

h2. Audience

This template is initially aimed at fairly experienced developers. If this is your very first Ruby project, then this template probably is not for you. This is alpha-grade software. As the work progresses, it should become a viable choice for less experienced developers. 

h2. Features

* Packaging for Windows and Mac OSX. 
...
Note: Due to the way the packaging code works, you need to require everything in the environment file when the program starts up. There should be no 'requires' sprinkled throughout the code, or you take the chance of not having found by the packaging code. 
...
* Instead of a template, my aim is to create a script that will generate the required directory structure and generate the code that glues it all together. 
...
As with many open-source developer's I have a day job and this is a side project, please be patient. 
