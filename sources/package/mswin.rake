namespace :package do 
  ...
  task :mswin, "version"  do |t, args|
    ...
    puts `#{makensis} /V1 /DAPP_NAME=#{Environment.app_name} /DAPP_NAME_DOWNCASE=#{Environment.app_name.underscore} \
      /DAPP_DOC_EXTENSION=#{app_doc_extension} /DVERSION=#{build_version} /DVERSIONEXTRA=#{build_version + ".0.0"} \
       /DBUILD_DIR=#{build_dir} #{nsis_scriptfile_name}`
  end
  ...
  def generate_manifest_file
    File.open(manifest_file_name, 'w') do | f |
      f.puts("<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>" +
        ...
        "version=\"1.8.6.0\" processorArchitecture=\"X86\" name=\"Microsoft.Winweb.Ruby\" type=\"win32\" />" +
        "<description>#{Environment.app_name} Ruby Interpreter</description><dependency><dependentAssembly>" + 
        "<assemblyIdentity type=\"win32\" name=\"Microsoft.Windows.Common-Controls\" version=\"6.0.0.0\" " +
        "processorArchitecture=\"X86\" publicKeyToken=\"6595b64144ccf1df\" language=\"*\"  /></dependentAssembly>" +
        "</dependency></assembly>")
    end
  end
  ...
  def depends
    config["i386-mswin32"][:depends]
  end

  def find_dynamic_libraries(feature, current_libraries)
    return if feature =~ /\.rb$/
    `otool -L #{feature}`.grep(/\s+\//).inject(current_libraries) do  | libraries, line |
  ...
  def ruby_executable_name
    @ruby_executable_name ||= File.join( Config::CONFIG['bindir'],
      Config::CONFIG['RUBYW_INSTALL_NAME'] +
      Config::CONFIG['EXEEXT']
    )
  end

  def app_ruby_executable_name
    @app_ruby_executable_name ||=  Environment.app_name.underscore + '.exe'
  end
